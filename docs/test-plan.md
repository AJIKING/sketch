...
- ブラシプレビュー(ink / pencil / marker / air の筆跡。固定 seed で生成)— **実装済み** `test/golden/brush_preview_golden_test.dart`。
- カラーピッカー(SV/Hue)— **実装済み** `test/golden/color_picker_golden_test.dart`。
- (今後)ツールドック・レイヤー行。

> baseline は未生成(`golden.yml` を CI で実行して `test/golden/goldens/` に
> コミットするまで `golden` job は赤。ローカルは自動 skip で緑)。
